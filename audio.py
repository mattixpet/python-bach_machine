...
# Usage: [b, c] = getRandomBachSeedChord()
# Pre:   nothing
# Post:  b is a random seed chord for making a bach chord progression 
#        c is b's chord_type
def getRandomBachSeedChord():
    return 0
...
def getChordFrom(note, chordType):
    print "note: ", reprNote(note)
    order = getOrderSingle(note, chordType)
    print "order: ", order
    base = chordType.getBaseChord().getNotes()
    # append another chord to basechord
    # for example: base = [0, 4, 7, 10]
    # after loop: base = [0, 4, 7, 10, 12, 16, 19, 22]
    baseExt = []
    for num in base:
        baseExt.append(num+12)
    base.extend(baseExt)
    realChord = []
    # vandamalid er herna held eg!!!!!!!!
    cnt = 0
    for num in base:
        if cnt == 4:
            break
        if getOrderSingle(num, chordType) == order or cnt > 0:
...
def getOrder(inputChord, chordType):
    ...
    #if input chord is an array instead of a chord, that's okay!
    try:
